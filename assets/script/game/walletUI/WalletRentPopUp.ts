...
import { DatePicker } from '../common/common/datePicker/DatePicker';
...
@ccclass('WalletRentPopUp')
export class WalletRentPopUp extends Component {
    @type(DatePicker)
    ...
    @type(EditBox)
    editBoxYear: EditBox = null;
    @type(EditBox)
    editBoxMonth: EditBox = null;
    @type(EditBox)
    editBoxDay: EditBox = null;
    @type(WalletCard)
    ...
    param: WalletRentPopUpParam = null;
    tradeId: number = 0;
    start() {
    ...
    onAdded(param: WalletRentPopUpParam) {
        ...
        this.editBoxYear.enabled = false;
        this.editBoxMonth.enabled = false;
        this.editBoxDay.enabled = false;
        this.datePicker.init((data: { y: number, m: number, d: number }) => {
            this.editBoxYear.string = `${data.y}`;
            this.editBoxMonth.string = `${data.m}`;
            this.editBoxDay.string = `${data.d}`;
        });
        this.datePicker.node.active = false;
    }

    showDatePicker() {
        this.datePicker.node.active = true;
    }
    ...
    async confimClick() {
        ...
        let y = Number(this.editBoxYear.string);
        let m = Number(this.editBoxMonth.string);
        let d = Number(this.editBoxDay.string);
        let date = new Date();
        if (new BigNumber(price).lte(0) || isNaN(y) || isNaN(m) || isNaN(d) ||
            this.getRentalEndAt() < new Date().setUTCHours(0, 0, 0, 0) / 1000 + 24 * 60 * 60) {
        ...
        let gas = "0";
        let usd = "0";
        let _d = await HttpHome.queryGas("TradeRentReq", watrade.TradeRentReq.create({
            tokenId: this.tradeId,
            dnaPrice: CommonUtil.etherToWei(this.editBox.string).toFixed(),
            rentalEndAt: this.getRentalEndAt(),
        }));
        if (_d) {
        ...
        find('Node/priceNode/num', this.confirmNode).getComponent(Label).string = `${price}`;
        find('Node/poundageNode/num', this.confirmNode).getComponent(Label).string = gas;

        find('Node-001/node/num', this.confirmNode).getComponent(Label).string = `${y}/${m}/${d}`;
        find('Node/poundageNode/poundageLbl', this.confirmNode).getComponent(LanguageLabel).params = [
            {
                key: 'num1',
                value: `${1}`,
            },
    ...
    getRentalEndAt() {
        ...
        let m = Number(this.editBoxMonth.string);
        let d = Number(this.editBoxDay.string);
        var strtime = `${y}-${m}-${d} 00:00:00Z`;
        var date = new Date(strtime);
        let t = Math.round(date.getTime() / 1000);
        // t = t - 24 * 60 * 60;
        return t;
    }
    ...
    async rentClick() {
        let price = this.editBox.string;
        let t = this.getRentalEndAt();

        let nftId = this.tradeId;
        // let _d = await HttpHome.tradeRent(nftId, price, t);
        // if (_d) {
        //     // this.cardInfo.state = core.NftState.NftStateRenting;
        //     this.closeClick();
        // }
        let closeCB = () => {
            this.closeClick();
        ...
        HttpHome.tradeRent(nftId, price, t)
            .then((d: watrade.TradeRentResp) => {
            }).catch(async (e: { code: errcode.ErrCode, data: watrade.TradeRentResp }) => {
...
export type WalletRentPopUpParam = {
    card?: core.ICard,
