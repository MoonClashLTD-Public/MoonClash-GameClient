...
export default class ResLoader {
    ...
    public release(path: string | string[], bundleName: string = resources.name) {
    ...
    public releaseDir(path: string, bundleName: string = resources.name) {
        ...
        if (path == "" && bundleName != resources.name && bundle) {
            assetManager.removeBundle(bundle);
        }
    }

    public get<T extends Asset>(path: string, type?: __private._cocos_core_asset_manager_shared__AssetType<T> | null, bundleName: string = resources.name): T | null {
        var bundle: AssetManager.Bundle | null = assetManager.getBundle(bundleName);
    ...
    public dump() {
        ...
        console.log(`:${assetManager.assets.count}`);
    }
}
