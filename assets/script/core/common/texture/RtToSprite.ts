import { Camera, Component, EventTouch, gfx, Input, Node, RenderTexture, Sprite, SpriteFrame, UITransform, Vec3, _decorator } from 'cc';
...
export class RtToSprite extends Component {
    ...
    start() {
        ...
        if (this.rotation) {
            this.sprite!.node.on(Input.EventType.TOUCH_START, this.onTouchStart, this);
            this.sprite!.node.on(Input.EventType.TOUCH_MOVE, this.onTouchMove, this);
            this.sprite!.node.on(Input.EventType.TOUCH_END, this.onTouchEnd, this);
            this.sprite!.node.on(Input.EventType.TOUCH_CANCEL, this.onTouchEnd, this);
        }
    }
    ...
    onDestroy() {
        if (this.rotation) {
            this.sprite!.node.off(Input.EventType.TOUCH_START, this.onTouchStart, this);
            this.sprite!.node.off(Input.EventType.TOUCH_MOVE, this.onTouchMove, this);
            this.sprite!.node.off(Input.EventType.TOUCH_END, this.onTouchEnd, this);
            this.sprite!.node.off(Input.EventType.TOUCH_CANCEL, this.onTouchEnd, this);
        }
        this.rt.destroy();
    }
}
